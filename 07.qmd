...
8.  래스터 `L7_ETMs`의 벡터 표현을 사용하여 `POINT(293716 9113692)`를 중심으로 반지름 75 m인 원형 영역과의 교차를 플로팅하고, 이 원의 면적-가중 평균 픽셀 값을 계산하시오. 벡터 데이터를 이용해 애그리게이션한 값과 래스터 데이터를 이용해 애그리게이션한 값(`exact=FALSE` 및 `exact=TRUE` 각각)을 비교하고, 차이점을 설명하시오.
