...
데이터 큐브(data cube)는 시간에 걸쳐 일련의 지오메트리 특성을 반복적으로 관찰할 때 자연스럽게 형성된다. 시간 정보는 건물의 건축 연도나 사람의 생년월일을 기록하는 것처럼 피처의 속성으로 간주될 수 있다(5장 참조). 그러나 시간 정보가 속성을 관측한 시점이나 해당 속성에 대한 예측 시점을 나타내는 경우도 있다. 이러한 경우, 시간은 공간과 동등한 위상을 가지며, 시간과 공간은 함께 우리가 관찰ㆍ모형화ㆍ예측ㆍ예보하는 물리적 차원을 설정한다.
