...
또한, 여러 변수라는 의미가 동일한 변수의 여러 시점을 가리키는 경우에도 다변량(코크리깅) 예측 방법을 적용할 수 있다. 하지만 이 경우 두 시점 사이의 임의 시점에 대해 직접적으로 인터폴레이션하는 것은 불가능하다. 이러한 상황이나, 여러 시간 인스턴스에서 관측된 데이터를 처리해야 하는 경우에는 $Z(s,t)$와 같이 연속적인 시공간 *결합* 함수를 통해 변동성을 모형화할 수 있다. 다음 절에서 이를 다룬다.
...
최근에 시공간 지구통계 데이터의 처리 및 모형화에 관한 현대적인 접근을 다룬 책들이 출간되었다. 예를 들어 Wikle, Zammit-Mangion, & Cressie(2019), Blangiardo & Cameletti(2015) 등이 있다. 여기서는 Gräler, Pebesma, & Heuvelink(2016)를 참고하며, 이전 장에서 사용한 데이터셋을 바탕으로 간단한 예제를 제시한다.
...
이 샘플 베리오그램에는 특정한 베리오그램 모형을 적합할 수 있다. 여기서는 비교적 유연한 모형인 곱합(product-sum) 모형(Gräler, Pebesma, 및 Heuvelink 2016)을 적용하며, 그 적합 과정은 다음과 같다.
...
![샘플 베리오그램은 곱합 모형 적합 결과](https://r-spatial.org/book/13-Geostatistics_files/figure-html/fig-prodsummodelplot-1.png){#fig-13-2}

![적합된 시공간 베리오그램 모형의 와이어프레임 플롯](https://r-spatial.org/book/13-Geostatistics_files/figure-html/fig-modelwire-1.png){#fig-13-3}

시공간 베리오그램의 적합 전략과 대체 모향에 대해서는 Gräler, Pebesma, & Heuvelink(2016)를 참고하라.
