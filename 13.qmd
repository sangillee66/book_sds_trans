...
이 데이터셋에 대해 추가적인 선택을 수행하였다. 측정된 시간별 값 중 75% 이상이 유효한 측정소만을 선택한 것이다. 즉, 결측 시간별 값이 25%를 초과하는 측정소는 제외하였다. `mean(is.na(x))` 함수는 벡터 `x`에서 결측값의 *비율*(fraction)을 계산하므로, 이 함수를 각 열(측정소)에 적용하면 된다.
...
다변량 지구통계학은 여러 변수를 *결합*하여 모형화, 예측, 시뮬레이션하는 것을 의미한다. 이를 수식으로 표현하면 다음과 같다.
